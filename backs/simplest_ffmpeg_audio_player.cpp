...
void  fill_audio(void *udata,Uint8 *stream,int len){ 
	...
	printf("fill_audio. len %d, audio_len %d\n",len, audio_len);
...
//			return -1; 
	...
	while(av_read_frame(pFormatCtx, packet)>=0){
		if(packet->stream_index==audioStream){
			...
			int resConv = 0;
			if ( got_picture > 0 ){
				resConv = swr_convert(au_convert_ctx,&out_buffer, MAX_AUDIO_FRAME_SIZE,(const uint8_t **)pFrame->data , pFrame->nb_samples);
#if 1
				printf("index:%5d\t pts:%lld\t packet size:%d \t samples converted: \n",index,packet->pts,packet->size, resConv);
#endif
